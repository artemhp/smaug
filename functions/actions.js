...
function getCoordinates() {
  ...
  var parseCoordinates = getCoordinateNode(getTravelFrame()).innerHTML.match(
    /(\d+, \d+)/gi
  );
...
function chooseCoordinates(getArrows) {
  ...
  getArrows.sort(function (a, b) {
    if (a.coordinateRate > b.coordinateRate) return 1;
    if (a.coordinateRate < b.coordinateRate) return -1;
    return 0;
  });
...
function searchTreasure() {
  ...
  if (picks) {
    if (picks.innerHTML !== "") {
      ...
      if (foundItemNode) {
        var countFoundItem = parseInt(
          picks.querySelector("input[type=text]").value
        );
        return {
          item: foundItemNode,
